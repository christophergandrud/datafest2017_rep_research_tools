...
In this workshop we will **create a simple fake research project** using the literate programming tool R Markdown.
...
R Markdown allows us to not only make writing HTML easier but also $\LaTeX$.
...
Note: `r` is just one of the "engines" you can use with RMarkdown. Others include `python`, `Bash`, and `ruby`.
...
Set the table code chunk in *main.Rmd* to `echo=FALSE`.
...
For example, this chunk sources an R source code file stored on the internet. It scapes a table from the BBC and includes creates a table with the results.
...
Place the following code in *main.Rmd*:
